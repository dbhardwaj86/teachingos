...
> llm_client.py` (the ONE LLM call site, Phase D2) is now **provider-aware**: `SAMAGRA_LLM_PROVIDER` ∈
> {anthropic (default, byte-identical D2 back-compat), openai}. OpenAI path uses the Responses API
> (`openai>=2.32`), default model `gpt-5.5`, validated `SAMAGRA_LLM_EFFORT` (none/minimal/low/medium/high/xhigh,
> fail-closed), strict `json_schema` output, `_extract_json_openai` with the same never-leak-content hardening;
> `configured()` + new `required_key_var()` are provider-aware; samadhan preflight now names the selected
> provider's key var. `requirements.txt` += `openai>=2.32`; `.env.example` gained a provider block
> (`SAMAGRA_LLM_MODEL` is now an inert override-only knob). Live `.env`: provider=openai, gpt-5.5, effort
> medium, key present — interface pre-declared by the Chairman.
> - **⭐ Live smoke:** `SAMAGRA_LIVE_LLM_SMOKE=1` was RUN FOR REAL — **PASS, ~52s**, genuine `gpt-5.5` generation
> + adversarial-review round-trip on `circular-motion`, artifacts written. This closes the D2-era owner
> follow-up that was never runnable (no Anthropic key ever arrived) — first-ever live validation of the D2
> generation boundary end to end.
> - **Review gate:** per-task 2-lens (spec + quality) = PASS/PASS on all 4 plan tasks. Dedicated Codex pre-merge
> review 32 (the DEC-7-style generation-boundary review) = **GO-WITH-CAVEATS**: M (`.env.example`'s active
> `claude-opus-4-8` model line masked the OpenAI default for template followers) + L (preflight refusal message
> hardcoded `ANTHROPIC_API_KEY`) — **both closed TDD same-slice** (`42ce9d3`, `e3ae45f`); the named finding
> (`SAMAGRA_LLM_MODEL` could name a non-reasoning model while `reasoning.effort` is always sent) adjudicated
> **accept-as-operator-responsibility** (explicit override; OpenAI API fails closed cleanly at build time).
> Effectively **GO**. Report `docs/codex-reviews/32-llm-provider-openai-premerge.report.md`.
> - **Invariants HELD:** keys env-only from the gitignored `.env`, never logged/repr'd/echoed (both providers) ·
> fail-closed on unknown provider/effort/missing key; `configured()` False → preflight refusal, never a wedge ·
> **DEC-8 reviewer firewall structural on BOTH providers** (`review_samadhan` never receives StyleSeed) · no new
> prod write path · publish gate untouched · no migration · rollback = `SAMAGRA_LLM_PROVIDER=anthropic` (or
> unset) restores D2 exactly.
> - **Gate: 713 pytest** (0 failures, 2 skips = opt-in live smokes) **+ 639 vitest baseline** (frontend
> untouched).
> - **Spec** `docs/superpowers/specs/2026-07-07-samagra-llm-provider-openai-design.md` (Status flipped to
> SHIPPED, §6 records the implementation-outcome chronicle).
> - **▶ NEXT: Phase F** per DEC-9's ratified ordering: **image-gen figures first, slides second, NO audio.**
> `/learn` public exposure remains the separate owner deploy step. **DEC-8 invariants unchanged.**
>
> ---
>
...
> `rmdir /S /Q`.
> - **▶ NEXT:** (superseded above) the LLM provider mini-slice shipped 2026-07-07; **Phase F** per DEC-9's
> ratified ordering is next: **image-gen figures first, slides second, NO audio.**
>
> ---
>
...
> ---
>
> **▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶ ✅ PHASE G5 (FACTORY RUN OVER HTTP) SHIPPED 2026-07-06 — built TDD on branch `feature/factory-run-http`, review gate CLOSED (Codex 30 GO + addendum GO · 4-lens adversarial: 3 MED remediated + independently re-verified), DEC-14 RATIFIED, merged `--ff-only` to `main` + pushed, server restarted (the 3 endpoints + the Factory-run panel are LIVE).**
...
> - **DEC-14 RATIFIED 2026-07-06** by the Chairman's explicit merge-gate go ("1) ratify commit. 2) merge 3) push.") after the review gate closed (Codex 30 GO + addendum GO; 4-lens 3 MED remediated + re-verified). The spec's Status header flipped to RATIFIED in this same dedicated ratify commit — DEC-13's actual precedent, now repeated for G5. Text in the decisions block.
> - **Gate: 667 pytest** (666 passed, 1 skip = opt-in live-LLM smoke, 0 failures) + **639 vitest** (75 files); `tsc --noEmit` + `npm run build` green.
> - **Codex review 30 (dedicated pre-merge, `docs/codex-reviews/30-g5-factory-run-http-premerge.report.md`): GO** (main review, 0 HIGH/MED/LOW) **+ addendum GO** (1 LOW on the `336d670` `approve-seed` remediation — the scan+filter of in-review children ran BEFORE `_FACTORY_RUN_LOCK`, so a concurrent duplicate POST could capture stale ids and 409 instead of returning an idempotent-safe empty batch). **LOW fixed:** the scan+filter now runs INSIDE the lock, immediately before the approve loop, with a TDD concurrent regression test (`tests/test_api_factory_approve_seed.py::test_concurrent_double_post_does_not_409`) that failed 5/5 pre-fix (`[200, 409]`) and passes repeatably post-fix. Also closed: the existing mcd-named test now actually plants a mcd `seed` row (defense-in-depth coverage, not just seed_ref-prefix coincidence).
> - **⚠ REMAINING BEFORE MERGE (explicitly NOT done yet):** a 4-lens adversarial final review (firewall/write-mechanism · security · spec-fidelity · separate-entity) → remediate any findings TDD → re-run the full gate → `merge --ff-only` to `main` → push.
...
> - **Review-gate outcomes folded back into the slice:** the 4-lens review's 3 MED were remediated pre-merge — (1) HTTP `approve-seed` now scans in-review children INSIDE `_FACTORY_RUN_LOCK` and **skips llm/mcd kinds** (loops the CLI's own `run.approve` per deterministic child; a CLI-planned samadhan row for the same seed stays in-review — the llm/mcd lanes are CLI-only at BOTH HTTP gates now, commit `336d670` + the Codex-addendum LOW fix `83921a1`); (2) the DEC-14 ratification overclaim was corrected then properly ratified (below); (3) golden thread 1 now builds ALL 5 lanes over HTTP and publishes via the G3 endpoint, proving the full GUI recipe end-to-end.
> - **⚠ Chairman directive 2026-07-06 (question bank):** SAMAGRA's question bank should use **`C:\SandBox\claude_khanak_box\combinedDBQues`** (the combined question-DB project) as its source. Today the `paper`/`drill` lanes + the Questions app read the QX engine on `:8783`; rewiring to combinedDBQues is a **future slice** needing its own design + DEC-7-style review (it touches the read-only firewall's QX seam and the Phase-E coverage graph's concept spine).
> - **▶ NEXT (at G5 time): the first GUI-driven throughput run** at `/` (Publish app): Plan → Approve seed → Build all → Publish, verified at `/learn`. *(Update: **DONE 2026-07-06** — `gauss-law`, all four gates clicked in the Publish app, verified at `/learn` incl. the G4 adaptive loop; see the top banner.)* Then the combinedDBQues question-bank rewire (directive above) and Phase F (the heavy async LLM lanes) per DEC-9's ratified ordering.
>
> ---
>
...
11. **DEC-14 · G5 factory-run-over-HTTP invariants — RATIFIED 2026-07-06 (source: spec §9).** Ratified by the
    Chairman's explicit merge-gate go ("1) ratify commit. 2) merge 3) push.") after the full review gate closed
    (Codex 30 GO + addendum GO · 4-lens 3 MED remediated + independently re-verified · 666 pytest + 639 vitest);
    the spec's Status header was flipped to RATIFIED in the same dedicated ratify commit (DEC-13's precedent). Note:
    post-review, invariant (3) is enforced at BOTH gates — HTTP approve-seed also skips llm/mcd children (commit
    `336d670`), not just build's 403. (1) **No new write
