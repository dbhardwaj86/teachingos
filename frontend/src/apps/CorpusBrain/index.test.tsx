// T3.2 — Corpus Brain corpus app: same-origin iframe of the gated serve proxy.
...
describe("CorpusBrain corpus app", () => {
  it("renders an iframe pointing at the corpus serve endpoint", async () => {
    render(<CorpusBrain />);
    await flush();
    const frame = screen.getByTitle("Corpus Brain") as HTMLIFrameElement;
    expect(frame.tagName).toBe("IFRAME");
    expect(frame.getAttribute("src")!.startsWith("/api/corpus/onedpull/serve/")).toBe(true);
    ...
    expect(vi.mocked(fetch).mock.calls[0][0]).toBe("/api/corpus/onedpull");
  });
  ...
  it("shows an offline fallback with Retry when the daemon is down", async () => {
    ...
    expect(screen.getByText(/:8137/)).toBeInTheDocument();
    expect(screen.queryByTitle("Corpus Brain")).toBeNull();
    // Retry re-probes and swaps the iframe back in when the daemon is up
    vi.stubGlobal("fetch", vi.fn(okFetch));
    fireEvent.click(screen.getByRole("button", { name: /retry/i }));
    await flush();
    expect(screen.getByTitle("Corpus Brain")).toBeInTheDocument();
  });
});
