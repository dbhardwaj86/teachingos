// T3.2 — Lecture Brain corpus app: same-origin iframe of the gated serve proxy.
...
import LectureBrain from "./index";
...
describe("LectureBrain corpus app", () => {
  it("renders an iframe pointing at the corpus serve endpoint", async () => {
    render(<LectureBrain />);
    await flush();
    const frame = screen.getByTitle("Lecture Brain") as HTMLIFrameElement;
    expect(frame.tagName).toBe("IFRAME");
    expect(frame.getAttribute("src")!.startsWith("/api/corpus/lecturepdf/serve/")).toBe(true);
    ...
    expect(vi.mocked(fetch).mock.calls[0][0]).toBe("/api/corpus/lecturepdf");
  });
  ...
  it("shows an offline fallback with Retry when the daemon is down", async () => {
    ...
    expect(screen.queryByTitle("Lecture Brain")).toBeNull();
    // Retry re-probes and swaps the iframe back in when the daemon is up
    vi.stubGlobal("fetch", vi.fn(okFetch));
    fireEvent.click(screen.getByRole("button", { name: /retry/i }));
    await flush();
    expect(screen.getByTitle("Lecture Brain")).toBeInTheDocument();
  });
});
